...
const S = {
  ...
  // 5색 스포트라이트(중심·외곽 넓게 hold 18% · 중간 3색 균등 16%). 신규 2색=기존 사이 보간.
  // Codex 로즈: #581b3a → #892e54 → #ba426f → #bf6a94 → #c593b9
  hero: 'radial-gradient(116% 124% at 52% 46%, #581b3a 0%, #581b3a 18%, #892e54 34%, #ba426f 50%, #bf6a94 66%, #c593b9 82%, #c593b9 100%)',
  // 날짜 세이지: #0d0a1b → #212430 → #363e44 → #646f6f → #92a19a
  media: 'radial-gradient(74% 76% at 50% 44%, #0d0a1b 0%, #0d0a1b 18%, #212430 34%, #363e44 50%, #646f6f 66%, #92a19a 82%, #92a19a 100%)',
  mediaSel: 'radial-gradient(74% 76% at 50% 44%, #1a1530 0%, #1a1530 18%, #323548 34%, #4a565f 50%, #798686 66%, #a8b6ae 82%, #a8b6ae 100%)',
  // 오늘 웜로즈: #2c0f1f → #59213d → #86345c → #ae678a → #d69ab8
  mediaToday: 'radial-gradient(74% 76% at 50% 44%, #2c0f1f 0%, #2c0f1f 18%, #59213d 34%, #86345c 50%, #ae678a 66%, #d69ab8 82%, #d69ab8 100%)',
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', background: 'transparent', color: S.text, fontFamily: "'Pretendard','Nunito',-apple-system,sans-serif" }}>
      ...
      <div className="no-scrollbar" style={{ position: 'relative', zIndex: 4, padding: '4px 20px 120px' }}>
        ...
        {shots.length >= 3 && (
          <div style={{ marginTop: 18 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', margin: '0 2px 7px' }}>
              ...
              <span onClick={() => go('/lounge')} style={{ fontSize: 11, fontWeight: 800, color: S.accentText, cursor: 'pointer' }}>더 보기 →</span>
            </div>
            <div className="no-scrollbar" style={{ display: 'flex', gap: 7, overflowX: 'auto', paddingBottom: 2 }}>
              {shots.map((sh, i) => (
                <div key={i} onClick={() => setShotOpen(sh)}
                  style={{ position: 'relative', width: 104, height: 130, flexShrink: 0, borderRadius: 16, overflow: 'hidden', cursor: 'pointer', border: `1px solid ${S.border}`, background: S.glass }}>
        ...
        {nextPlan && (
          <div onClick={() => go(`/student/records?date=${nextPlan.date}${nextPlan.cls ? `&class=${encodeURIComponent(nextPlan.cls)}` : ''}`)} style={{ marginTop: 14, borderRadius: 20, padding: '14px 16px', background: S.glass, ...glassBlur, boxShadow: S.hi, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 800 }}>다음 시간엔</div>
              <div style={{ fontSize: 11, color: S.sub, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{nextPlan.text}</div>
            </div>
            <span style={{ flexShrink: 0, fontSize: 11, fontWeight: 800, color: S.accentText, background: S.accentSoft, borderRadius: 10, padding: '5px 11px' }}>{nextPlan.teacher ? `${nextPlan.teacher} 쌤` : '강사'}</span>
          </div>
        )}
