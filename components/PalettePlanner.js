...
const tint=(c,w)=>{const a=w==null?0.5:w;return {h:c.h,s:c.s*(1-a),l:c.l+(100-c.l)*a}}   // w=화이트 농도(기본 0.5, 최대 0.8)
...
function moodPalette(P,key,tw){
  ...
  if(key==='warm')  return{ dom:tint(P[warmr],tw), sec:{h:P[warmr].h,s:cl(P[warmr].s*0.62,30,64),l:cl(P[warmr].l-4,34,58)}, acc:{h:warm(V.h)>0?V.h:P[warmr].h,s:cl(V.s,58,90),l:56} }
...
function derive(P,mix,ratio,domCrit,accentSel,blackMix,domSel,moodSel,tintW){
  const pureAt=makePureAt(P,mix),pure=[],tin=[]
  for(let i=0;i<12;i++){const p=pureAt(i*30);pure.push(p);tin.push(tint(p,tintW))}
  ...
  if(moodSel){const mp=moodPalette(P,moodSel,tintW);domBase=mp.dom;secBase=mp.sec;accBase=mp.acc}
  else{domBase=P[domI];secBase=P[secI];accBase=acc}
  const domGroup=blackMix?[tint(domBase,tintW),domBase,shade(domBase)]:[tint(domBase,tintW),domBase]
  const secGroup=blackMix?[secBase,shade(secBase)]:[secBase,tint(secBase,tintW)]
...
function drawWheelOn(ctx,cx,cy,R,D,tintW){
  ...
  for(let i=0;i<12;i++){const a0=(i*30-15-90)*Math.PI/180,a1=(i*30+15-90)*Math.PI/180,pu=D.pureAt(i*30)
    wedge(ctx,cx,cy,rTint,rPure,a0,a1,cssH(pu));wedge(ctx,cx,cy,rBand,rTint,a0,a1,cssH(tint(pu,tintW)))}
...
function drawCard(ctx,W,H,D,P,mood,ratioStr,domLabel,tintW){
  ...
  drawWheelOn(ctx,W/2,300,168,D,tintW)
...
export default function PalettePlanner({ initial: providedInitial, role, saving, onClose, onSave, locked, onSignup }){
  ...
  useEffect(()=>{ const c=cv.current; if(!c)return; drawWheelOn(c.getContext('2d'),120,120,100,D,tintW) },[D,tintW])
  ...
  async function save(){
    ...
    drawCard(c.getContext('2d'),720,1000,D,P,mood,ratioStr,domLabel,tintW)
    const blob=await new Promise(res=>c.toBlob(res,'image/png'))
    const palette={ v:1, primaries:P, mix, ratio, domCrit, blackMix, domSel, moodSel, tintW, accentManual:!!accentSel, accent:D.acc,
        ...
        <div style={{ padding:'14px 0 0', position:'relative' }}>
          ...
          <section style={section}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px 14px', marginTop:16 }}>
              {[['바깥 · 순색',cssH(D.pureAt(0))],[`중간 · +화이트 ${Math.round(tintW*100)}%`,cssH(tint(D.pureAt(0),tintW))],['띠 · 3원색 순회색',cssR(D.gray)],['중앙 · 회색+화이트',cssR(D.ctr)]].map(([t,bg])=>(
            ...
            <div style={{ display:'flex', alignItems:'center', gap:12, marginTop:15 }}>
              <span style={{ font:`600 11px ${FSANS}`, color:TX2, flexShrink:0 }}>틴트 화이트</span>
              <input className="pp-range" type="range" min={20} max={80} step={5} value={Math.round(tintW*100)} onChange={e=>setTintW(+e.target.value/100)}
                style={{ flex:1, background:`linear-gradient(to right, ${accent} 0%, ${accent} ${(Math.round(tintW*100)-20)/0.6}%, #2a2431 ${(Math.round(tintW*100)-20)/0.6}%)` }}/>
              <span style={{ font:`600 11px ${FMONO}`, color:'#EAE4F0', width:34, textAlign:'right', flexShrink:0 }}>{Math.round(tintW*100)}%</span>
            </div>
          </section>
          ...
          <section style={section}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:7, marginTop:9 }}>
              {D.pure.map((c,i)=><button key={'p'+i} onClick={()=>pickAccent(c)} style={{ height:46, borderRadius:9, border:'none', background:cssH(c), cursor:'pointer', padding:0, boxShadow: brickSel(c)?`0 0 0 2px ${BG}, 0 0 0 4px #F0ECF4`:'none' }}/>)}
            </div>
            <div style={{ marginTop:18, font:`600 11px ${FMONO}`, letterSpacing:'0.06em', color:MUT }}>틴트 · +화이트 {Math.round(tintW*100)}%</div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:7, marginTop:9 }}>
              {D.tin.map((c,i)=><button key={'t'+i} onClick={()=>pickAccent(c)} style={{ height:46, borderRadius:9, border:'none', background:cssH(c), cursor:'pointer', padding:0, boxShadow: brickSel(c)?`0 0 0 2px ${BG}, 0 0 0 4px #F0ECF4`:'none' }}/>)}
            </div>
          </section>
          ...
          <section style={section}>
            ...
            <div style={{ marginTop:15, display:'grid', gridTemplateColumns: blackMix?'30px 1fr 1fr 1fr':'30px 1fr 1fr', gap:'10px 7px', alignItems:'center' }}>
              ...
              {P.map((c,i)=>{const cells=blackMix?[shade(c),c,tint(c,tintW)]:[c,tint(c,tintW)];return (
                <Fragment key={i}>
                  <div style={{ font:`700 11px ${FMONO}`, color:TX2 }}>P{i+1}</div>
                  {cells.map((cc,j)=>(
                    <button key={j} onClick={()=>pickAccent(cc)} style={{ border:'none', background:'none', padding:0, cursor:'pointer', textAlign:'left' }}>
          ...
          <section style={section}>
            ...
            <div style={{ marginTop:14 }}>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gridAutoRows:'30px', gap:6 }}>
                ...
                <div style={{ gridColumn:'2 / span 2', gridRow:'1 / span 3', background:cssH(tint(D.dom,tintW)), borderRadius:8 }} title="주체"/>
                <div style={{ gridColumn:4, gridRow:1, background:cssH(tint(D.sec,tintW)), borderRadius:6 }}/>
                {ratio==='622' ? (<>
                  <div style={{ gridColumn:4, gridRow:2, background:cssH(D.sec), borderRadius:6 }} title="부수"/>
                  <div style={{ gridColumn:4, gridRow:'3 / span 2', background:cssH(D.acc), borderRadius:6 }} title="종속"/>
                </>) : (<>
              ...
              <div style={{ display:'flex', gap:14, marginTop:11, flexWrap:'wrap' }}>
                {[['주체',cssH(tint(D.dom,tintW)),D.wd[0]],['부수',cssH(D.sec),D.wd[1]],['종속',cssH(D.acc),D.wd[2]]].map(([n,bg,w],i)=>(
