...
// 일시정지 — 상태색과 구분되는 중립 슬레이트(경고 아님, 잠시 멈춤)
export const PAUSE = { main: '#5B7A99', tx: '#3E5B79', soft: '#E9EFF5', dot: '#5B7A99' }

// 시작 전 — 수강권은 받았지만 수업 시작일이 아직 안 옴(대기, 경고 아님)
export const UPCOMING = { main: '#7A6BB0', tx: '#4F4287', soft: '#EEEAF8', dot: '#7A6BB0' }

// 회원 수강권 상태 → 팔레트
// ⚠️ getStatus(회원 관리)가 돌려주는 상태를 하나라도 빠뜨리면 그 회원이 있는 순간 목록 화면 전체가 멈춘다
//    (2026-09-19 실사고: '시작 전' 누락). 새 상태를 만들면 여기에 먼저 추가할 것.
export const MST = { '수강중': OK, '만료임박': WARN, '만료': BAD, '일시정지': PAUSE, '시작 전': UPCOMING }
