...
export const FARM_CATS = [
  ...
  // 우주냥 — 스페이스 테마 연동. 2달 한정 무료, 이후 작물 교환 해금.
  { key: 'astronaut', name: '우주냥', img: '/farm/body-helmet-visor2.png', act: 'water', desc: '물주기 · 토마토', crop: 'tomato', cropName: '토마토', unlock: { freeUntil: '2026-09-24', harvest: 15 } },
]

// 기간 한정 무료 여부(오늘 <= freeUntil).
function farmInFreeWindow(unlock, now = new Date()) {
  if (!unlock || !unlock.freeUntil) return false
  const ymd = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`
  return ymd <= unlock.freeUntil
}

// 기간 한정 무료 배지 라벨 — 설정 그리드에서 "2달 무료" 표기용
export function farmCatSeasonLabel(key, now = new Date()) {
  const c = FARM_CATS.find(x => x.key === key)
  return c && farmInFreeWindow(c.unlock, now) ? '2달 무료' : ''
}
...
export function farmCatUnlocked(key, { harvest = 0, unlockAll = false } = {}) {
  ...
  if (farmInFreeWindow(c.unlock)) return true // 기간 한정 무료(2달)
