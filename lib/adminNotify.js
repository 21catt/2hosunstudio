...
export async function staffIdsForCourse(courseId, scheduleId = null) {
  // ⚠️ 승인 대기 관리자는 제외한다. 가입만 하면 role 이 admin 이라, 안 거르면 승인도 받기 전에
  //    예약·문의 알림(학생 이름·연락처가 들어 있다)이 그 계정으로 쌓인다 —
  //    심지어 자기 가입 신청 알림을 자기가 받는다.
  const { data: owners } = await supabase.from('users').select('id, approved').eq('role', 'admin')
  const ids = new Set((owners || []).filter(o => o.approved !== false).map(o => o.id))
  ...
  if (courseId) {
    // 수업 담당 + 보조 담당(중복 배치) — co_teacher_ids 컬럼이 없으면 조용히 담당만
    let { data: c } = await supabase.from('class_courses')
      .select('teacher_id, co_teacher_ids').eq('id', courseId).maybeSingle()
    if (!c) ({ data: c } = await supabase.from('class_courses')
      .select('teacher_id').eq('id', courseId).maybeSingle())
    if (!teacherId) teacherId = c?.teacher_id || null
    // 보조 담당은 타임 담당이 따로 있어도 함께 받는다(그 수업에 붙어 있는 사람들이다)
    ;(c?.co_teacher_ids || []).forEach(id => { if (id) ids.add(id) })
  }
