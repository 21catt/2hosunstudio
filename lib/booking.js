...
// 수업 시작일이 지났는가 — 시작일 규칙의 단일 소스.
// ⚠️ 유효 판정은 화면마다 관용도가 다르다(냥밭은 만료일이 없어도 유효로 본다).
//    그 차이는 그대로 두고 시작일 조건만 이 함수로 공유한다 — 세 곳이 각자 판정하면
//    "홈에서는 쓸 수 있는데 캘린더에서는 안 되는" 상태가 생긴다.
// nil = 시작일 없음 = 부여 즉시 시작(기존 수강권).
export function ticketStarted(ticket, todayStr) {
  if (!ticket?.start_date) return true
  return ticket.start_date <= todayStr
}

// 유효한(시작일 이후·기간 내·잔여 있는) 수강권 여부
