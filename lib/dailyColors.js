...
// 순서 = 계절 흐름으로 배열하되, 시작을 여름에 두었다(도입 시점이 한여름이라 지금 계절과 맞물리게).
// 여름 → 가을 → 겨울 → 봄 → (다시 여름). 명도·채도·한난 수치를 근거로 정렬했다.
//   여름: 고채도·시원한 원색 (형광·시안·터콰이즈)
//   가을: 중저명도·난색 (갈색·와인·주황)
//   겨울: 저명도·한색/무채 (네이비·딥그린·회색)
//   봄  : 고명도·중채도 파스텔 (연두·연블루·연노랑) → 다시 여름으로
...
// 순환 기준일 — 이 날짜에 목록 0번(여름 첫 조합)이 나오고, 하루에 한 칸씩 이동한다.
const ANCHOR = Date.UTC(2026, 6, 18) // 2026-07-18
