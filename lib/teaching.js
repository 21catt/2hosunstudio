...
//
// 담당은 두 층 + 보조다:
//   · class_courses.co_teacher_ids 에 있으면 → 그 수업의 보조 담당(알림·범위 포함)
...
export async function loadTeachingScope(uid) {
  // 주담당 + 보조 담당(중복 배치). co_teacher_ids 컬럼이 없는 환경이면 주담당만(기존과 동일).
  let { data: courses } = await supabase.from('class_courses')
    .select('id').or(`teacher_id.eq.${uid},co_teacher_ids.cs.{${uid}}`)
  if (!courses) ({ data: courses } = await supabase.from('class_courses').select('id').eq('teacher_id', uid))
